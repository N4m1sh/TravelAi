...
// const GooglePlacesInput = ({ handleInputChange }) => {
//   const [place, setPlace] = useState("");
...
//       placeholder="Enter a City"
//       className="h-12 w-full rounded-md border border-gray-300 px-4 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
//     />
//   );
// };
...
function CreateTrip({ createTripPageRef }) {
  ...
  const SaveUser = async () => {
    ...
    const id = storedUser.email;
    await setDoc(doc(db, "Users", id), {
      userName: storedUser.name,
      userEmail: storedUser.email,
      userPicture: storedUser.picture,
      userNickname: storedUser.nickname,
    });
  };
  ...
  const generateTrip = async () => {
    ...
    if (days > 5 || days < 1) {
      return toast.error("Trip days must be between 1 and 5.");
    }
  ...
  return (
    <div ref={createTripPageRef} className="mt-10 text-center">
      <h2 className="text-3xl md:text-5xl font-bold mb-5">Share Your Travel Preferences</h2>
      <p className="opacity-90 mx-auto text-md md:text-xl">Plan your dream trip with ease.</p>

      <div className="form mt-14 flex flex-col gap-16 md:gap-20">
        {/* Location */}
        <div className="place">
          <h2 className="font-semibold text-lg md:text-xl mb-3">Where do you want to Explore?</h2>
          <ReactGoogleAutocomplete
            apiKey='AIzaSyAcjJpFAGuurK5cbNZg-RrsmTH0PSX_h7g'
            onPlaceSelected={(selectedPlace) => {
            ...
            options={{ types: ["(cities)"] }}
            placeholder="Enter a City"
            className="h-12 w-full border border-gray-300 px-4 text-lg text-black bg-white focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="day">
          ...
          <Input
            className="text-center border border-gray-300 text-black"
            placeholder="Ex: 2"
            type="number"
            min="1"
            max="5"
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{user ? "Thank you for Logging In" : "Sign In to Continue"}</DialogTitle>
            <DialogDescription>
              {user ? "" : (
                <Button onClick={SignIn} className="w-full mt-5 flex gap-2 items-center justify-center">
                  Sign In with <FcGoogle className="h-5 w-5" />
                </Button>
              )}
            </DialogDescription>
